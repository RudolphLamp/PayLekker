...
class PayLekkerSetup {
    ...
    private function step2_CreateDatabase() {
        ...
        try {
            ...
            // Remote database failsafe - use pay.sewdani.co.za hosted database
            $this->printWarning("Local database failed. Attempting remote database failsafe...");
            
            // Update configuration to use remote database
            $this->config = [
            ...
            try {
                ...
                $this->printSuccess("✅ Remote database failsafe connection established!");
                $this->printSuccess("   Using hosted database at pay.sewdani.co.za");
                
                // Update the database.php file with remote credentials
                $this->updateDatabaseConfig();
                
            } catch (PDOException $remoteError) {
                $this->printError("Both local and remote database connections failed:");
                $this->printError("Local error: " . $e->getMessage());
                $this->printError("Remote error: " . $remoteError->getMessage());
                throw new Exception("Database connection failed on both local and remote hosts");
            }
        }
    }
    
    /**
     * Update database configuration file with current settings
     */
    private function updateDatabaseConfig() {
        $configContent = "<?php\n";
        $configContent .= "// PayLekker Database Configuration\n";
        $configContent .= "// Auto-generated by setup.php\n\n";
        $configContent .= "\$host = '{$this->config['host']}';\n";
        $configContent .= "\$dbname = '{$this->config['dbname']}';\n";
        $configContent .= "\$username = '{$this->config['username']}';\n";
        $configContent .= "\$password = '{$this->config['password']}';\n";
        $configContent .= "\$charset = '{$this->config['charset']}';\n\n";
        $configContent .= "// Connection string for PDO\n";
        $configContent .= "\$dsn = \"mysql:host=\$host;dbname=\$dbname;charset=\$charset\";\n";
        
        file_put_contents(__DIR__ . '/src/database.php', $configContent);
        $this->printSubStep("✓ Database configuration updated in src/database.php");
    }
